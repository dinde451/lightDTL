...
void MainmenuLoad(const char *name)
{
...
#ifndef NOEXIT
	vecMenuItems.push_back(std::make_unique<UiListItem>(gbIsHellfire ? _("Exit Diablo") : _("Exit Diablo"), MAINMENU_EXIT_DIABLO));
#endif

	if (!gbIsSpawn || gbIsHellfire) {
		if (gbIsHellfire)
			ArtBackgroundWidescreen = LoadOptionalClx("ui_art\\mainmenuw.clx");
		LoadBackgroundArt("ui_art\\menu");
	} else {
	...
	if (gbIsHellfire) {
		...
		vecMainMenuDialog.push_back(std::make_unique<UiArtText>(_("- Theodore's Lair -").data(), rect1, UiFlags::FontSize24 | UiFlags::ColorRed | UiFlags::AlignCenter, 8));
	}

	vecMainMenuDialog.push_back(std::make_unique<UiList>(vecMenuItems, vecMenuItems.size(), uiPosition.x + 64, (uiPosition.y + 192), 510, 43, UiFlags::FontSize30 | UiFlags::ColorUiGold | UiFlags::AlignCenter, 5));
