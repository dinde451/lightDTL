...
uint8_t FindShadedColor(int colorIndex, int shade);
...
void palette_update_quest_palette(int n)
{
	...
	uint8_t FindShadedColor(int colorIndex, int shade)
{
    double ratio = std::log(16);
    double fraction = std::log(shade);
    SDL_Color blendedColor;
    blendedColor.r = ((double)orig_palette[colorIndex].r / ratio * fraction);
    blendedColor.g = ((double)orig_palette[colorIndex].g / ratio * fraction);
    blendedColor.b = ((double)orig_palette[colorIndex].b / ratio * fraction);

    return FindBestMatchForColor(orig_palette, blendedColor, -1, -1);
}
